...
int nmea_gen_GPRMC(char *buff, int buff_sz, nmeaGPRMC *pack)
{
    return nmea_printf(buff, buff_sz,
        ...
        pack->utc.day+3, pack->utc.mon, pack->utc.year - 2000,
...
int nmea_gen_GPHDT(char *buff, int buff_sz, nmeaGPHDT *pack)
{
	return nmea_printf(buff, buff_sz,
	        "$GPHDT,%.1f,%C",
	        pack->headingdeg, pack->truedeg);
}
...
void nmea_info2GPHDT(const nmeaINFO *info, nmeaGPHDT *pack)
{
    nmea_zero_GPHDT(pack);

    pack->headingdeg = info->heading;
    pack->truedeg = 'T';
}
    ...
    )
{
    ...
    while(pack_mask)
    {
        ...
        else if (pack_mask & GPHDT)
        {
        	nmea_info2GPHDT(info, &hdt);
        	gen_count += nmea_gen_GPHDT(buff + gen_count, buff_sz - gen_count, &hdt);
			pack_mask &= ~GPHDT;
        }
        else
            break;
