...
extern "C" void mavlink_ParseBinary(unsigned char *message, unsigned int length);
...
//	uint16_t idx;
//
//	save_file = fopen("save/190722_124546_rangeb.txt", "rb");
//	if (save_file == NULL)
...
//
//	while (fread(message, sizeof(signed char), 1024, save_file) != 0){
//
...
////				best_xyz.x_velocity, best_xyz.y_velocity, best_xyz.z_velocity);
//
//		printf("++ Position[ECEF] %8.2f %8.2f %8.2f %d %d\n",
//				best_xyz.x_position, best_xyz.y_position, best_xyz.z_position,
//				best_xyz.number_of_satellites_in_solution, best_xyz.solution_status);
//
//		if (best_position_ecef_callback_)
//			best_position_ecef_callback_(best_xyz, read_timestamp_);
//		break;
	case BESTVELB_LOG_TYPE:
		Velocity vel;
		memcpy(&vel, message, sizeof(vel));

		bestVel[0] = vel.horizontal_speed;
		bestVel[1] = vel.track_over_ground;

		break;
	case PSRDOPB_LOG_TYPE:
		Dop dop;
		memcpy(&dop, message, sizeof(dop));
		psrdop = dop.horizontal_dop;
	...
	case BESTPOSB_LOG_TYPE:
		...
		double sinlat, coslat, sinlon, coslon, alt, rn, x, y, z;
		sinlat = sin(best_pos.latitude * 3.1415926535898/180);
		coslat = cos(best_pos.latitude * 3.1415926535898/180);
		sinlon = sin(best_pos.longitude* 3.1415926535898/180);
		coslon = cos(best_pos.longitude* 3.1415926535898/180);
		alt = best_pos.height;

		rn = 6378137.0 / sqrt(1 - (2-1/298.257223563)*1/298.257223563 * sinlat * sinlat);
		bestPos[0] = (rn + alt) * coslat * coslon;
		bestPos[1] = (rn + alt) * coslat * sinlon;
		bestPos[2] = (rn * (1 - (2-1/298.257223563)*1/298.257223563) + alt) * sinlat;
		numsat[0] = best_pos.number_of_satellites;
		numsat[1] = best_pos.number_of_satellites_in_solution;

//		printf("-- Position[ECEF] %8.2f %8.2f %8.2f \n",x,y,z);
...
//		Eph_curr.GPS[idx].status		= gps_ephemeris.s
	...
	case TIMEB_LOG_TYPE :
		TimeOffset timeutc_;
		memcpy(&timeutc_, message, sizeof(timeutc_));

		timeutc[0] = timeutc_.utc_year;
		timeutc[1] = timeutc_.utc_month;
		timeutc[2] = timeutc_.utc_day;
		timeutc[3] = timeutc_.utc_hour;
		timeutc[4] = timeutc_.utc_minute;
		timeutc[5] = timeutc_.utc_millisecond;
...
bool Novatel::Connect_mav(std::string port, int baudrate, bool search) {

	bool connected = Connect_mav_(port, baudrate);
...
bool Novatel::Connect_mav_(std::string port, int baudrate = 115200) {
	try {
		...
		std::stringstream output;
		output << "Error connecting to MavLink on com port " << port << ": "
				<< e.what();
		log_error_(output.str());
		is_connected_ = false;
		return false;
...
void Novatel::StartReading_mav() {
	if (reading_status_)
		return;
	// create thread to read from sensor
	reading_status_ = true;
	read_thread_ptr_ = boost::shared_ptr<boost::thread>(
			new boost::thread(boost::bind(&Novatel::ReadSerialPort_mav, this)));
}
...
void Novatel::ReadSerialPort_mav() {
	unsigned char buffer[MAX_NOUT_SIZE];
	static int msg_id;
	long signed int len;
	...
	while (reading_status_) {
		...
		mavlink_ParseBinary(buffer, len);
	}
}
...
//		printf("- %3i %3i %3i %3i\n ",ii, message[ii], buffer_index_, bytes_remaining_);
...
//			printf("remaining bytes : %d  length : %d \n", bytes_remaining_, length);
			...
			// Save Mode - Log binary data
			if (SysMode_Save) {
				char SaveFileName[50];
				sprintf(SaveFileName, "%s/onboard_%s.gps", LogDirName, start_time);
...
//		printf("- [ii]%3i [val]%3i [bin]%3i [bi]%3i [flag]%3i ",
//				ii, message[ii], buffer_index_nmea, buffer_index_, nmea_flag);
...
//				 printf(" :%c", data_buffer_nmea_[buffer_index_nmea]);
				 buffer_index_nmea++;
			 } else {
				 data_buffer_nmea_[buffer_index_nmea] = message[ii];
//				 printf(" - parsed");
